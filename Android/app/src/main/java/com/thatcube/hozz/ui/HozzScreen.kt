    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        LazyColumn(
            ...
            item {
                ...
                Text(
                    text = "Downloadable beta · Not a Google Play release or approval. " +
                        "Hozz moves records; it does not provide medical advice. " +
                        "Keep your original archive.",
...
private fun ProjectionPreview(
    ...
    var confirmsWrite by remember { mutableStateOf(false) }
    if (confirmsWrite) {
        AlertDialog(
            onDismissRequest = { confirmsWrite = false },
            title = { Text("Apply experimental Health Connect changes?") },
            text = {
                Text(
                    "Insert ${plan.insertCount} and update ${plan.updateCount} mapped records " +
                        "in Health Connect. Other apps you authorize there may read them. " +
                        if (plan.deleteCount > 0) {
                            "Also delete ${plan.deleteCount} records previously written by Hozz. " +
                                "Tombstones remain in your Hozz archive. " +
                                "Only continue if you approve these writes and deletions."
                        } else {
                            "Nothing is written until you continue and grant the requested access. " +
                                "Your complete archive stays in Hozz."
                        },
                )
            },
            confirmButton = {
                Button(
                    ...
                    Text(if (plan.deleteCount > 0) "Write and delete" else "Continue to write")
                }
            },
            dismissButton = {
                TextButton(onClick = { confirmsWrite = false }) {
                    Text("Cancel")
                }
            },
        )
    ...
    Card(
        ...
        Column(Modifier.padding(18.dp)) {
            Text(
                text = "Experimental Health Connect",
                style = MaterialTheme.typography.titleMedium,
                fontWeight = FontWeight.SemiBold,
            )
            Row(
