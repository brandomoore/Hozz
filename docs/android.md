# Android archive and experimental projection

The project under `Android/` is a native Hozz archive shell with a downloadable
sideload-beta packaging path. The beta explicitly includes experimental Health
Connect writes; this is not a Google Play release or policy approval. See
[beta onboarding and release requirements](android-beta.md). A configured build
pipeline alone does not mean a signed download has been published.
...
The beta manifest still declares write permissions for steps, distance,
...
while their mappings remain unsafe. This beta has no supported migration that
guesses at untracked prototype writes: deleting an
unknown Health Connect record would violate the same idempotency rule this
archive implementation is establishing. The beta presents explicit write
consent before permissions, and explicit write/delete consent when needed;
cancelling does not request permission or mutate Health Connect.
...
Android stages a complete export in app-private storage before publishing
through a SAF **document tree** selected with the folder picker. Only the local
external-storage provider (on-device/SD-card folders) is supported. Hozz creates
a uniquely named temporary ZIP, closes/syncs and verifies its byte count and
SHA-256, renames it to a unique final name, then verifies again. An existing
document is never replaced. Cloud/proxy providers are rejected before creating
a document because safe local publication cannot be guaranteed. Failed
publication deletes only the newly created document; cleanup failures are
...
is included as an experimental, explicitly opted-in capability in the
downloadable beta. This choice does not resolve Google Play policy review for
user-initiated Apple Health migration; no Play listing or approval is claimed.
